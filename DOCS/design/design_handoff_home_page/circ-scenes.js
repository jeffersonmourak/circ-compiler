...
/** index.astro's heroSource: input a, b · xor s · and c · output sum, carry. */
export function halfScene({ a = 1, b = 1 } = {}) {
  const s = a ^ b, c = a & b;
  const V = (v) => val(v, 1);
  const pinA = makeComponent("input_pin", { id: 1, name: "a", x: 0, y: 1 });
  const pinB = makeComponent("input_pin", { id: 2, name: "b", x: 0, y: 7 });
  // xor is a builtin macro on the site, not a primitive — the --preview
  // figure on the same page prints it as [xor:s].
  const xor = makeComponent("subcircuit", { id: 3, name: "s", subcircuit: "xor", x: 12, y: 0, macroInputs: 2 });
  const and = makeComponent("and_gate", { id: 4, name: "c", x: 12, y: 6 });
  const oS = makeComponent("output_pin", { id: 5, name: "sum", x: 12 + xor.width + 2, y: 1 });
  const oC = makeComponent("output_pin", { id: 6, name: "carry", x: 12 + xor.width + 2, y: 7 });
  pinA.value = V(a); pinB.value = V(b); xor.value = V(s); and.value = V(c); oS.value = V(s); oC.value = V(c);
  xor.inValues = [V(a), V(b)]; and.inValues = [V(a), V(b)]; oS.inValues = [V(s)]; oC.inValues = [V(c)];
  const P = (k, i) => k.inPorts[i].coord, O = (k) => k.outPort;
  const wires = [
    { srcId: 1, dstId: 3, dstPort: "a", value: V(a), segments: [h(O(pinA).x, 8, 2), v(8, 2, P(xor, 0).y), h(8, P(xor, 0).x, P(xor, 0).y)], crossings: [] },
    { srcId: 1, dstId: 4, dstPort: "a", value: V(a), segments: [h(O(pinA).x, 8, 2), v(8, 2, P(and, 0).y), h(8, P(and, 0).x, P(and, 0).y)], crossings: [] },
    { srcId: 2, dstId: 3, dstPort: "b", value: V(b), segments: [h(O(pinB).x, 9, 8), v(9, 8, P(xor, 1).y), h(9, P(xor, 1).x, P(xor, 1).y)], crossings: [] },
    { srcId: 2, dstId: 4, dstPort: "b", value: V(b), segments: [h(O(pinB).x, 9, 8), v(9, 8, P(and, 1).y), h(9, P(and, 1).x, P(and, 1).y)], crossings: [] },
    { srcId: 3, dstId: 5, dstPort: "in", value: V(s), segments: [h(O(xor).x, P(oS, 0).x, 2)], crossings: [] },
    { srcId: 4, dstId: 6, dstPort: "in", value: V(c), segments: [h(O(and).x, P(oC, 0).x, 8)], crossings: [] },
  ];
  return { components: [pinA, pinB, xor, and, oS, oC], wires, gridW: oC.x + oC.width + 1, gridH: 11 };
}

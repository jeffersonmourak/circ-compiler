...
/** roundRect whose radius is clamped at 0 — at cell ≤ 4 several
 *  (w − gap·(n−1)) / n corner terms dip below zero and the API throws. */
function nsRR(ctx, x, y, w, h, r) {
  const rr = Array.isArray(r) ? r.map((v) => Math.max(0, v)) : Math.max(0, r);
...
export const siteSkins = {
  ...
  subcircuit: ({ ctx, t, cell, c, inSigs, outSig }) => {
    ...
    if (usingSprite) {
      ...
      ctx.beginPath();
      nsRR(ctx, x0 + cell * 0.08, y0 + cell * 0.08, w - cell * 0.16, h - cell * 0.16, cell * 0.25);
      ctx.fill();
      ctx.stroke();
...
function nsSliceAsset(ctx, t, cell, c, inSigs, outSig) {
  ...
  for (let i = 0; i < inWidth; i++) {
    ...
    ctx.beginPath();
    nsRR(ctx, tx, ry, tickW, rulerH, Math.min(tickW / 2, cell * 0.07));
    if (taken) {
...
function nsConcatAsset(ctx, t, cell, c, inSigs, outSig) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    ctx.fillStyle = t.wireBus;
    ctx.beginPath();
    nsRR(ctx, barX, segY, barW, segH, cell * 0.07);
    ctx.fill();
    ctx.globalAlpha = 1;
...
function nsMemory(ctx, t, cell, c, inSigs, outSig, mode) {
  ...
  ctx.strokeStyle = t.macro;
  ctx.lineWidth = Math.max(1.75, cell * 0.1);
  ctx.beginPath(); nsRR(ctx, bx, y0, bw, h, r); ctx.fill(); ctx.stroke();
  ctx.save();
  ctx.beginPath(); nsRR(ctx, bx, y0, bw, h, r); ctx.clip();

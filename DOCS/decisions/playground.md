...
**Decision.** `rootOf(files)` remains `files[files.length - 1]`, and the tab strip marks that tab with a `root` badge. The model has no root field, and none is added: moving a tab past the end is the way to change which file is compiled. `requestFor` roots the request on the same entry, so the model, the badge, and the compiler always agree.

**Rationale.** One source of truth beats two that can disagree. A separate root pointer would need validation on every add, delete, and rename, and a stale pointer compiles the wrong file with no visible cause.
...
**Rationale.** Status text assembled inline at eight call sites cannot be reasoned about — the question is which state wins when several are true at once, and that is answerable only if the precedence lives in one place a test can drive. Keeping the live region quiet is the other half: a region that re-announces on every keystroke is worse than no region at all.
...
**Alternatives.** One debounce with one counter (the shipped behaviour before this phase); cancelling in the worker (the worker has no abort, and adding one means a session protocol); blanking the outputs on every failed build (the flicker this phase exists to remove).
...
**Decision.** A classic inline script at the top of the playground markup stashes the fragment on a global and immediately calls `history.replaceState` to remove it, inside `try/catch`. The island reads the stashed copy. It then walks load precedence one rule at a time — a share fragment, a `#pick=` id, the project last open, the first catalogue entry — with each failure falling through to the *next* rule and posting a note, never skipping to the default.

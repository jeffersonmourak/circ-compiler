...
**Decision.** Compilation runs as `.circ source → langlang-generated Zig parser (lib/parser/parser.zig) → resolved IR → `circ.topology.v0.{min,full}` binary payloads → custom sections appended to pre-built runtime blob → `.wasm``. The CLI spawns no `zig` subprocess at circuit-compile time.
...
**Rationale.** The CLI is a single self-contained binary — no install prefix, no runtime path lookup, no version drift. Reproducibility is built in: the same CLI binary always embeds the same runtime version. Concurrent `circ-compile` invocations are safe by construction — they share no mutable filesystem state. The runtime is built with `-Dwasm-optimize` (default `ReleaseSmall`, stripped) independently of `-Doptimize`, and the `libcirc` library embeds a wasm-target twin of the same bytes — see [libcirc.md](libcirc.md) "`-Dwasm-optimize` governs both wasm builds".

...
The CLI driver is `cmd/circ-compile/main.zig`. It dispatches several mutually exclusive modes; `run()` is the authoritative set, and the [README usage table](../README.md#usage) documents what each produces. Only the default mode produces a `.wasm`. `main()` handles a separate `--analyze` invocation ahead of `run()`: it reads a JSON request on stdin (rather than a file path) and emits structured diagnostics, symbols, and references for editor tooling such as the external circ-lsp server. See [analyze-api.md](analyze-api.md).
...
`lib/circuit.zig` defines nine kinds (`ComponentType`):

| Kind             | Inputs                                  | Output port | Notes                                                              |
|------------------|-----------------------------------------|-------------|---------------------------------------------------------------------|
| `input_pin_gate` | `"in"` (sub-circuit only)               | `"out"`     | The host drives top-level input pins via `setPin`.                 |
...
| `slice`          | `"in"`                                  | `"out"`     | Bit-shape kind: masks bits `[lo, hi)` of `from`. Output width is `hi - lo`. The resolver lowers `a[lo..hi]` and `a[i]` into it; users never write it directly. |
| `concat`         | `"operand_0"`, `"operand_1"`, … per op  | `"out"`     | Bit-shape kind: ORs each operand into its bit-position slot. Output width is the sum of operand widths. The resolver lowers `{a, b, ...}` into it. |
...
The runtime template is the WASM shell that ships embedded inside every compiled artifact. It lives in `templates/main.zig` and `templates/interpreter.zig`, and `zig build` compiles it once into `zig-out/lib/circ-runtime.wasm`. `build.zig` generates a `runtime_embed` module (a `@embedFile` of the prebuilt artifact) into which `lib/libcirc/modes.zig` splices the topology, so users need no Zig toolchain at runtime.
...
- Imports two log callbacks from the host (`debugEnabled`, `onDebugLog`) — that's it. No `onStateChange` callback exists; hosts poll `getOutputValue` / `getOutputDefined` after `run()`.
...
- **`lib/topology/serializer.zig`** writes the compact `.min` blob: a flat ordered list of primitive components and their connections. Sub-circuits are fully flattened: the runtime sees no hierarchy.
- **`lib/topology/full_serializer.zig`** writes the `.full` blob: per-file IDs, port names, component aliases, and macro provenance for tooling that needs human-readable structure.
- **`lib/emit/`** is the experimental `--emit-zig` pipeline: it generates standalone Zig source that compiles to a `.wasm` with a richer (but unstable) export surface (`getStateSnapshot`, `getFileInfo`, `freeBuffer`, …). Default compile never takes this path.
...
This build has no "TypeScript SDK" target, Canvas-2D rendering layer, or `compiler:run` step; the CLI-only model replaced those prototypes. The canvas that draws circuits today is the separate `circ-renderer` package, which the site pins.

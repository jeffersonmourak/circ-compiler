...
**Decision.** `run()` drains the event queue synchronously and returns when the circuit has settled, subject to the engine's bounded settling budget. The API offers no continuous-time mode and no host-driven tick loop.

**Failure boundary.** Each settle allows 1,000,000 work units across event pops, inertial candidate validations, downstream evaluations, and rejected-target recalculations. Exhaustion poisons the runtime until re-instantiation; void drive/run exports trap, and the additive `getSimulationStatus()` reports `1`. This prevents older hosts from silently accepting partial results while newer hosts can identify `E_NOSETTLE` separately from other traps. The playground uses the renderer's public constructor with that status query, preserving one shared runtime for canvas, Data, console, and agent actions. Gate-based latches remain legal; AND and NOT transitions use inclusive inertial delay so stale equal-deadline pulses are rejected before commit. The budget remains the backstop for genuine oscillation, not a hard wall-clock deadline or static proof.
...
**Decision.** `setPin(component_id, value, defined)` accepts the **input pin's** component ID. The paired `getOutputValue(component_id)` / `getOutputDefined(component_id)` getters accept the **driver** component ID — that is, the ID of the component whose `out` port feeds the `output` declaration, not the `output_pin`'s own ID. The API has no separate pin-index space.

**Rationale.** One identifier scheme is simpler than two. Component IDs are dense `0..nodes.len` integers assigned by `Circuit.createComponent` and stable for the life of a compiled artifact (the topology serializer fixes them at emission time), so they're as stable as a separate pin index would be. The `circ.topology.v0.full` custom section carries the mapping from declaration name to component ID (each record carries its name and kind); `circ-compile --inspect` prints, under each module's `Inputs (...)` / `Outputs (...)` block, resolver-local ids that coincide with the artifact's only for a single-file circuit without sub-circuits (see "`--inspect` prints widths, not global ids" below).
...
**Decision.** `setPin(component_id: i32, value: i64, defined: i64) -> void`. The host writes the same `(value, defined)` shape it reads. The engine silently masks bits beyond the input pin's declared width.
...
**Decision.** The compiled artifact is purely pull-based — after `run()` returns, the host polls outputs via the paired getters. The artifact has no `onStateChange` import, no event queue exposed to the host, and no listener registration. The only host imports are `debugEnabled` (returns whether log emission is wanted) and `onDebugLog` (receives UTF-8 log buffers).
...
**Rationale.** WASM custom sections are opaque to the module itself — the module has no API for reading them. The three-call protocol (`topology_alloc` → `memcpy` → `init`) is the smallest portable bridge any JS host can implement without an SDK. Hosts that don't need rendering metadata can ignore the parallel `circ.topology.v0.full` section, which carries the same structural data plus per-component names and macro provenance.
...
- `deinit()`, `reset()`, `stop()` — lifecycle controls. Re-instantiating the module covers every use case for `reset`/`deinit`; `stop` would matter only for non-settle-only run modes that don't exist.
...
- `freeBuffer()` — companion to the introspection exports above; not needed because no export returns an owned buffer.

A richer surface (`getStateSnapshot`, `getFileInfo`, `freeBuffer`, …) still exists in `lib/emit/runtime.zig`, the experimental `--emit-zig` pipeline. That path is not on the default compile and its export contract is unstable.

The list above records *omissions*, not a promise that the six original exports are the whole surface: topology v03 added the memory export family (`getMemInfo`, `memBuffer`, `memLoad`, `memStore`, `memClear`, `setMemWord`, `getMemValue`, `getMemDefined`) as the first additive extension — see `## Native memories` below.
...
**Decision.** A memory's cells live in two heap-allocated planes on the component payload, `values: []u64` and `defined: []u64`, one entry per address (`2^A` entries, `A ≤ 16`), allocated in `createComponent`. Cells stay out of the width-tiered state pools.

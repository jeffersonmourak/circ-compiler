...
pub fn emitRuntimeExports(allocator: std.mem.Allocator, module: *const ir.Module) ![]u8 {
    ...
    try writer.writeLine("var circuit: engine.Circuit = undefined;");
    try writer.writeLine("var component_table: []const *engine.Component = &.{};");
    try writer.writeLine("");
    try writer.writeLine("fn emptyPtrLen() PtrLen {");
    writer.indent();
    try writer.writeLine("return .{ .ptr = @ptrFromInt(0), .len = 0 };");
    writer.dedent();
    try writer.writeLine("}");
    try writer.writeLine("");
    ...
    try writer.writeLine("circuit = engine.Circuit.init() catch return;");
    try writer.writeLine("_ = buildCircuit(&circuit) catch {");
    writer.indent();
    try writer.writeLine("circuit.deinit();");
    try writer.writeLine("return;");
    writer.dedent();
    try writer.writeLine("};");
    try writer.writeLine("component_table = circuit.nodes.items;");
    ...
    writer.indent();
    try writer.writeLine("if (!runtime_initialized) return;");
    try writer.writeLine("circuit.propagate() catch return;");
    writer.dedent();
    try writer.writeLine("}");
    try writer.writeLine("");
    ...
    try writer.writeLine("circuit.propagateEvent(component_table[id], engine.State.fromInt(state)) catch return;");
    writer.dedent();
    try writer.writeLine("}");
    try writer.writeLine("");
    ...
    try writer.writeLine("const encoded = circuit.encodeState() catch return emptyPtrLen();");
    ...
    try writer.writeLine("const mutable: [*]u8 = @constCast(ptr);");

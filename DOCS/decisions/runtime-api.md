...
**Decision.** `run()` drains the event queue synchronously and returns when the circuit has settled. The API offers no continuous-time mode and no host-driven tick loop.

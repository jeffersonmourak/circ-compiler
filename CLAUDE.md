...
6. **The allocator is global, not parameterised.** Allocations route through `memory.allocator` from `lib/memory.zig`: an `ArenaAllocator` over `page_allocator` on every target, so `free` only rewinds the most recent allocation and repeated allocation grows linear memory monotonically (allocate once and reuse, as the runtime's per-memory staging buffers do). `memory.reset()` (`arena.reset(.free_all)`) exists for libcirc and is called only with no `Circuit`/`Session` alive. Do not add an allocator parameter to engine functions.

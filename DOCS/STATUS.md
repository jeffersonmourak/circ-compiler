...
## 2026-05-02 — Phase 9 — Slice 9.7 getting-started doc

**What shipped:** New **`DOCS/getting-started.md`** (~110 lines) walking a fresh user through: (1) install + verify by running **`circ-compile --inspect`** on **`tests/fixtures/circuits/inverter.circ`**; (2) writing a first **`examples/inverter.circ`** (input → not → led + output); (3) compiling it with **`circ-compile -o`** and using **`--inspect`** to discover the input pin's component id and the **driver** id of the output (the v0 export semantics use the driver id, not the **`output_pin`** id, for **`getOutputState`** — called out explicitly in the doc); (4) loading from Node via a copy-paste **`run.mjs`** that supplies the required **`env.debugEnabled`** / **`env.onDebugLog`** stubs and demonstrates **`init`** / **`setPin`** / **`run`** / **`getOutputState`** / **`deinit`** with expected stdout `a=0 -> NOT a = 1` / `a=1 -> NOT a = 0`; (5) a built-in **`xor`** example that uses an explicit **`import xor "<builtin>/xor.circ"`** with a **v0 papercut** callout (single-file mode does not auto-import built-ins; explicit virtual-path import puts the file on the project pipeline); (6) a sub-circuit composition example using the half-adder source verbatim from **`tests/fixtures/projects/half_adder/`**. The doc closes with pointers to **`DOCS/circuit-format.md`**, **`DOCS/wasm-api.md`**, **`DOCS/architecture.md`**, **`DOCS/decisions/`**, and the canonical fixtures under **`tests/fixtures/`** as copy-and-modify templates.

Also wired **`DOCS/index.md`** to surface **`getting-started.md`** as the first row of the document table.

Verified each invocation manually before committing the doc copy: inspect on inverter.circ matches the documented head-of-output; **`circ-compile examples/inverter.circ -o examples/inverter.wasm`** produces a **`.wasm`**; the Node loader returns the documented **`0`** / **`1`** values for the inverter; the **`<builtin>/xor.circ`** explicit-import workaround compiles cleanly; **`circ-compile tests/fixtures/projects/half_adder/root.circ -o ...`** succeeds.

**Files touched:** `DOCS/getting-started.md`, `DOCS/index.md`, `DOCS/STATUS.md`

**Tests:** none — documentation. Ran **`zig build test`** to confirm no incidental changes — result **pass**.

**Next slice:** v0 sign-off — Phase 9 definition-of-done check ( **`zig build test`** green; every diagnostic code has a snapshot; canonical, edge, stress, and regression fixtures committed; smoke perf test in tree; **`README.md`** + **`DOCS/getting-started.md`** present). After human commit, no further phase work remains in **`DOCS/PLANS_PROMPT.md`**'s Phase index.

**Notes — v0 papercut tracked.** Single-file CLI mode skips **`scan_imports`**, so a file using **`or`**/**`nand`**/**`nor`**/**`xor`**/**`xnor`** without any user **`import`** raises **`E001`**. The doc's **`<builtin>/`** explicit-import workaround is honest documentation of v0 behaviour; v1 should run the project pipeline unconditionally so single-file built-ins parse without ceremony. **Discovery of root pin component ids in projects** (e.g., **`xor_demo`**'s output driver lands at **`id=18`** because the imported **`xor`** sub-circuit's interior gates fill the flat layout first) is currently best done either by scanning **`getOutputState(i)`** for **`i in 0..N`** in JS or by decoding the **`getFileInfo()`** binary blob; a programmatic helper would be a v1 ergonomic improvement. Examples paths in the doc (**`examples/inverter.circ`** etc.) are illustrative — there is no shipped **`examples/`** directory yet; copy from **`tests/fixtures/`** if you want a starting point.

## 2026-05-02 — v0 finalize — Rename to `circ-compiler` and switch to GPL-3.0

**What shipped:** Renamed the umbrella project across user-facing surfaces (**`circ-renderer-z`** → **`circ-compiler`**; npm **`name`** **`circ-renderer`** → **`circ-compiler`**; repo URLs in **`package.json`** updated to **`github.com/jeffersonmourak/circ-compiler`**) and updated the **`compiler_version`** metadata embedded in every emitted artifact (**`circ-renderer-z/dev`** → **`circ-compiler/dev`**) at both the CLI entry points (**`cmd/circ-compile/main.zig:203,212`**) and every test that invokes the emitter directly (**`tests/emit/{full,project,project_behavior,metadata,behavior}_test.zig`**, **`tests/orchestrator/main_test.zig`**). Regenerated affected golden outputs that bake the version string and its length into a binary blob via **`UPDATE_GOLDENS=1 zig build test`** — the **`*_file_info.zig`** goldens encode the version in their state-snapshot byte arrays, so the length byte (**`19`** → **`17`**) and the ASCII bytes for the version string both shifted. Replaced **`LICENSE`** with the canonical **GNU GPL v3.0** text (29 June 2007), updated **`package.json`** **`"license"`** from **`CC-BY-4.0`** → **`GPL-3.0`** and **`README.md`** license pointer to **GNU General Public License v3.0**.

Renamed in: **`README.md`**, **`DOCS/index.md`**, **`DOCS/architecture.md`** (intro paragraph), **`DOCS/PLANS_PROMPT.md`**, every **`tests/fixtures/expected-zig/*.zig`** golden header that printed **`Compiler version: "circ-renderer-z/dev"`**.

**Intentionally not renamed.** Six surfaces still reference **`circ-renderer-lib`** / **`circ-renderer-lib.wasm`** (**`build.zig:79,154,163`**, **`DOCS/index.md`** quick-start TS snippet, **`DOCS/architecture.md`** layer table, **`DOCS/wasm-api.md`** ×3, **`DOCS/PLANS/PHASE_5_BUILD_ORCHESTRATION.md`**, **`DOCS/decisions/compiler-pipeline.md`**). That string is the binary identifier of the **legacy dynamic-API browser library** (the original **`createComponent`** / **`connect`** / **`propagateEvent`** runtime), which is a *separate artifact* from the compiler — distinct WASM target, distinct host contract, distinct demo path under **`example/`**. Renaming it would cascade into the TypeScript SDK (**`src/index.ts`**), the demo app under **`example/`**, and any consumers pointing to **`example/circ-renderer-lib.wasm`**, none of which were in scope for this rebrand. If the user wants the legacy library renamed too (e.g. **`circ-runtime-lib.wasm`** or to retire it entirely), that's a separate change.

**Files touched:** `README.md`, `package.json`, `LICENSE`, `DOCS/index.md`, `DOCS/architecture.md`, `DOCS/PLANS_PROMPT.md`, `DOCS/STATUS.md`, `cmd/circ-compile/main.zig`, `tests/emit/{full_emit,project_emit,project_behavior,metadata,behavior}_test.zig`, `tests/orchestrator/main_test.zig`, `tests/fixtures/expected-zig/{empty_ish,and_two_inputs,anonymous_nested,empty_ish_file_info,and_two_inputs_file_info,anonymous_nested_file_info}.zig`, `tests/fixtures/expected-zig/projects/{and_pair,deep_chain,diamond,nested_invert,passthrough_chain,same_name_half_adder}/main.zig`

**Tests:** ran **`UPDATE_GOLDENS=1 zig build test`** to refresh **`*_file_info.zig`** byte-array goldens, then **`zig build test`** clean — result **pass** (111 tests).

**Next slice:** v0 sign-off remains — same as previous entry. Awaiting human review/commit of this rebrand alongside the prior 9.7 doc.

**Notes:** Searched repo end-to-end for stale **`circ-renderer-z`** references; none remain outside **`.zig-cache`** / **`zig-out`** build artefacts. Bare **`circ-renderer`** matches now point exclusively at the legacy **`circ-renderer-lib`** browser library identifier (intentional — see above).

...
6. Open `example:ram-write-read` with image `a0a1a2a3a4a5a6a7a8a9aaabacadaeaf`; at address `0x5` first observe `q=0xa5/defined=0xff`, write `0x3c` with ordered `we=1, clk=0, clk=1`, observe `q=0x3c/defined=0xff`, then reset and explicitly drive the address again to observe restored preload `0xa5`. Confirm live memory, console correlation, reset semantics, and session provenance.

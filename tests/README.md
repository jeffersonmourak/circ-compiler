...
dev-loop suite. `zig build test-all` also runs `test-emit`, the slow emit-zig backend
smoke that spawns a nested `zig build wasm` per fixture; that fuller suite is what CI
runs. Behavioral WASM harness tests also require `node` on your `PATH`; set
`SKIP_WASM_E2E=1` to skip the Node-driven WASM e2e tests (CI does).
...
- `tests/fixtures/expected-inspect/`: expected `--inspect` dumps.
...
- `tests/fixtures/projects/`: multi-file project roots (`<dir>/root.circ` plus siblings).
- `tests/fixtures/preview/`: `--preview` goldens — `renders/`, the text `layouts/`, the JSON `layouts-json/`, and `layout-invariants.golden`.
- `tests/fixtures/truth_table/`: `--truth-table` goldens.
- `tests/fixtures/bench/`: the engine benchmark golden and its history (`zig build bench`).
...
When adding a test, place the input circuit in `tests/fixtures/circuits/` and store each expected artifact in its matching `expected-*` directory. Reference fixtures by stable names in the test code so updates stay reviewable.

Running tests with `UPDATE_GOLDENS=1` regenerates the golden fixtures. In normal mode, tests compare actual output against existing fixtures.

Use consistent fixture names by feature: `<feature>.circ` with paired outputs of the same stem in the matching directory (`expected-zig/<feature>.zig`, `expected-wasm/<feature>.txt`, `expected-diagnostics/<feature>.txt`).

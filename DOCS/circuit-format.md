...
Compiled `.wasm` artifacts embed the resolved circuit as a custom section. The current format identifier is `CIRC` (v03). v02 added a `width: u8` byte to every serialised component (the runtime materialises the corresponding pool tier on `init()`); v03 adds the memory kinds `rom = 8` and `ram = 9`, whose records carry one trailing `addr_width` byte, and the port bytes `addr`/`din`/`we`/`clk`. The topology excludes memory contents; they are loaded at runtime. See [`wasm-api.md`](wasm-api.md) for the host-facing ABI and `lib/topology/serializer.zig` for the wire layout.

## Diagnostic Codes

`lib/validator/codes.zig` registers the stable validator surface (E001–E018, W001–W003); the rules behind each code are in [`language.md`](language.md) §4.1. The full catalogue follows, with each code's default message:

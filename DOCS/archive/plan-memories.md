...
The compiler *understands* `rom`/`ram` (IR, `--inspect`, `--analyze`, the validator contract) while every artifact-producing mode refuses them with `rom/ram are not yet supported in this mode`, exit 1, no partial output.

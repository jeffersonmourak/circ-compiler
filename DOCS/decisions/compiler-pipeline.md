...
**Decision.** The runtime (simulation engine, WASM entry point with `topology_alloc`, `init`, `run`, `setPin`, `getOutputState` exports) is compiled to `wasm32-freestanding` exactly once, at `zig build circ-compile` time, and embedded in the CLI binary via `@embedFile`.

...
pub fn place(
    ...
    const sizes = try arena.alloc(boxes.PrimitiveSize, n);
    for (graph.nodes, 0..) |node, i| sizes[i] = boxes.sizeOf(node, opts);
    ...
    for (graph.nodes, 0..) |node, i| {
        ...
        const ports = try boxes.resolvePortCoords(arena, node, x, y, w, h);

        const display_label = try boxes.composeDisplayLabel(arena, node, opts);

...
`.circ` files describe a digital circuit as a set of named components and their connections. The format is a simple declarative DSL that maps directly onto the simulation engine's component and connection model.
...
A `.circ` file contains a sequence of top-level declarations. Each declaration either names input pins or instantiates a component with named port connections. Every signal-carrying declaration may carry a width annotation `[N]`, where `N` is in `[1, 64]`; a missing `[N]` means width 1.
...
`input` is a sibling pin declaration with its own syntax (no port list), described above. All primitives accept an optional `[N]` width annotation; absent it, width is 1. Memories are the exception: they take exactly two width arguments, `[W, A]`, after the instance name.

**Built-in macro gates** expand at compile time to nested `and` / `not` (and optionally other macros). They use **`a`** and **`b`** as input ports and expose **`out`**. Unlike `and`/`not`, they need **no `import`**: the compiler behaves as if `import … from "<builtin>/<name>.circ"` were present. Each macro is parametric: `or[8] g(a=x, b=y)` produces a bit-parallel 8-bit OR.
...
- `<instance-name>.<port>` — the output pin of an imported subcircuit that exposes `<port>` (`sum`, `carry`, and so on).
...
You can define a component inline as the value of a port connection. The nested component has no instance name, but its `.out` output wires directly to the enclosing port:
...
Here an anonymous `not` gate drives port `b` of `combine`.
...
The compiled `.wasm` exposes no programmatic graph-construction API; the topology is baked into the artifact as a custom section and materialised by the embedded runtime at `init()`. Hosts see only the fixed export surface (`setPin`, `run`, paired `getOutputValue` / `getOutputDefined`, …) described in [`wasm-api.md`](/reference/wasm-api); the component IDs they need come from the `circ.topology.v0.full` section, where each record carries the component's name and kind. `circ-compile --inspect` prints resolver-local ids that match the artifact's only for a single-file circuit without sub-circuits.

## Grammar

The maintainer's langlang fork (`-output-language zig`) generates the parser from a PEG grammar at `lib/grammar/proto-circ.peg`; the generated file is vendored at `lib/parser/parser.zig`, and `lib/syntax/translate.zig` walks it.
...
Compiled `.wasm` artifacts embed the resolved circuit as a custom section. The current format identifier is `CIRC` (v03). v02 added a `width: u8` byte to every serialised component (the runtime materialises the corresponding pool tier on `init()`); v03 adds the memory kinds `rom = 8` and `ram = 9`, whose records carry one trailing `addr_width` byte, and the port bytes `addr`/`din`/`we`/`clk`. The topology excludes memory contents; they are loaded at runtime. See [`wasm-api.md`](/reference/wasm-api) for the host-facing ABI and `lib/topology/serializer.zig` for the wire layout.

## Diagnostic Codes

`lib/validator/codes.zig` registers the stable validator surface (E001–E018, W001–W003); the rules behind each code are in [`language.md`](/reference) §4.1. The full catalogue follows, with each code's default message:

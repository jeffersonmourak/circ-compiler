...
function boxOutline(ctx, t, cell, x, y, w, h, sig) {
  ...
  ctx.beginPath();
  ctx.roundRect(px + ctx.lineWidth / 2, py + ctx.lineWidth / 2, pw - ctx.lineWidth, ph - ctx.lineWidth, Math.max(0, r));
  ctx.fill();
  ctx.stroke();
}
...
const currentSkins = {
  ...
  subcircuit: ({ ctx, t, cell, font, c }) => {
    ...
    ctx.lineWidth = Math.max(1, cell * 0.12);
    ctx.beginPath();
    ctx.roundRect(x0 + ctx.lineWidth / 2, y0 + ctx.lineWidth / 2, w - ctx.lineWidth, h - ctx.lineWidth, Math.max(0, cell * 0.25));
    ctx.fill();
    ctx.stroke();
...
function hoverRing(ctx, t, cell, x, y, w, h, r) {
  ...
  ctx.globalAlpha = 0.9;
  ctx.beginPath();
  ctx.roundRect(x - cell * 0.3, y - cell * 0.3, w + cell * 0.6, h + cell * 0.6, Math.max(0, r + cell * 0.3));
  ctx.stroke();
  ctx.globalAlpha = 1;
}
...
const nextSkins = {
  input_pin: ({ ctx, t, cell, c, outSig, hovered }) => {
    ...
    const r = h / 2;
    const path = () => { ctx.beginPath(); ctx.roundRect(x, y, w, h, Math.max(0, r)); };
  ...
  output_pin: ({ ctx, t, cell, c, inSigs }) => {
    const sig = inSigs[0] ?? 2;
    const x = c.x * cell, y = c.y * cell, w = c.width * cell, h = c.height * cell;
    const path = () => { ctx.beginPath(); ctx.roundRect(x, y, w, h, Math.max(0, cell * 0.18)); };
  ...
  subcircuit: ({ ctx, t, cell, c }) => {
    const x0 = c.x * cell, y0 = c.y * cell, w = c.width * cell, h = c.height * cell;
    const path = () => { ctx.beginPath(); ctx.roundRect(x0, y0, w, h, Math.max(0, cell * 0.14)); };
    nextBody(ctx, t, cell, path, 0, { stroke: t.macro });
    ctx.save();
    ctx.beginPath();
    ctx.roundRect(x0, y0, w, h, Math.max(0, cell * 0.14));
    ctx.clip();
...
function iecBox(ctx, t, cell, c, sig, label, qualifier) {
  const x0 = c.x * cell, y0 = c.y * cell, w = c.width * cell, h = c.height * cell;
  const path = () => { ctx.beginPath(); ctx.roundRect(x0, y0, w, h, Math.max(0, cell * 0.1)); };
...
function busBadges(ctx, t, cell, comps, font, style) {
  ...
  for (const c of comps) {
    ...
    if (style === "current" || style === "site") {
      ...
      ctx.fillStyle = t.wireBus;
      ctx.beginPath();
      ctx.roundRect(cx - w / 2, y, w, h, Math.max(0, h / 2));
      ctx.fill();

...
### ROM images belong to source declarations

**Decision.** Image text is keyed by project, source file and memory name. File switches restore that file's images and their in-session filename/implied-zero metadata. File renames move the images, file deletion removes them, and project duplication or an example fork copies them. The version-2 envelope adds `sourceImages` with an empty default. Images fit only within the space left after saving sources; oversized images stay in memory, with a reload notice, rather than evicting source projects to meet the envelope cap. This replaces the earlier session-only image policy; share links and artifacts still carry source and topology, not image contents.

`SimSession` applies imported images at build, reset and preload reapplication. Each flattened ROM resolves its innermost origin frame's `targetFile` through analysis captured for the exact compile request, then looks up that source's declaration. Loading uses the component's runtime ID and validates against its compiled shape. Multiple instances receive independent copies. Concurrent analysis requests share a pending promise; a changed file-ID mapping replaces the session even when artifact bytes match.

**Rationale.** An image entered in `test.circ` must initialize its instances when `main.circ` imports it. Clearing images on selection lost the source data; retaining a name-only map would mix unrelated declarations. Root-only memory enumeration cannot find imported instances.

Truth uses a scratch session when sibling images are present, because the compiler accepts only root-memory preloads. Below the cap it enumerates every input combination, without a filtered label; above the cap it keeps the existing pin-filtered behavior. CSV and Markdown copies use those same rows. RAM anywhere in the topology refuses scratch enumeration. Console memory names remain root-only.

...
## 2026-05-04 — Phase 1 — Slice 3: Wire into circ-compile

**Status:** **COMPLETE**. Phase 1 ends here.

**What shipped:** `vendor/zig-compiler/build.zig.zon` and root `build.zig.zon` added to enable `b.dependency("zig_compiler", ...)` resolution. Root `build.zig` acquires the vendored module via `b.dependency("zig_compiler", .{ .target = target, .optimize = optimize })` and wires it into `circ_compile_mod` via `addImport("zig_compiler", ...)`. The MANIFEST was regenerated from 237 to 238 entries (adding `build.zig.zon`; `build.zig` hash updated because the module name was renamed from `"zig-compiler"` to `"zig_compiler"` in this slice to satisfy Zig 0.15.x's requirement for bare identifiers in `.name` fields). All pre-existing tests continue to pass; the vendored module is linked but not called at runtime.
**Files touched:** `vendor/zig-compiler/build.zig.zon` (new), `build.zig.zon` (new), `build.zig` (added 3 lines: dep acquisition + addImport), `vendor/zig-compiler/build.zig` (module name `"zig-compiler"` → `"zig_compiler"`), `vendor/zig-compiler/MANIFEST` (updated, 237 → 238 entries)
**Tests:** `sha256sum --check vendor/zig-compiler/MANIFEST` → 238 OK, exit 0. `zig build test` → all tests pass, exit 0.
**Next slice:** Phase 2 Slice 1, per `DOCS/PLANS/PHASE_2_integrate.md` (replace the `zig build` subprocess call with an in-process `Compilation.create()` / `comp.update()` call using the vendored `zig_compiler` module).

## 2026-05-04 — Phase 2 — Slice 1: Scaffold inprocess.zig

**What shipped:** `lib/orchestrator/inprocess.zig` created with the exact `compile(allocator, workspace_path, stderr_writer) !void` signature the plan specifies, returning `error.NotImplemented`. The stub references `zc.Compilation` (forces type-check against the vendored module when the file is compiled). Wired into `build.zig` as `inprocess_mod` with `zig_compiler` as an import; `orchestrator_main_mod.addImport("orchestrator_inprocess", inprocess_mod)` registers the import so Slice 3's `main.zig` change is a single-line addition.
**Files touched:** `lib/orchestrator/inprocess.zig` (new), `build.zig` (added `inprocess_mod` block + `addImport` to `orchestrator_main_mod`)
**Tests:** `zig build` → exit 0.
**Next slice:** Phase 2 Slice 2 — full `Compilation.create()` implementation in `inprocess.zig`; `tests/orchestrator/inprocess_test.zig` compiles a fixture, verifies WASM magic bytes.
**Notes:** Workspace input is `<workspace_path>/src/compiled.zig` (written by `workspace_mod.writeEmittedSource`). The root source for `Compilation.create()` must be `<workspace_path>/src/main.zig` (the re-export shim from `templates/main.zig` that does `comptime { _ = @import("compiled.zig"); }`). Output must land at `<workspace_path>/zig-out/bin/compiled.wasm` — that is what `finalize_mod.copyOutput` looks for. Configure `emit_bin = .{ .yes_path = "<workspace>/zig-out/bin/compiled.wasm" }` explicitly. The workspace also has `src/circuit.zig`, `src/memory.zig`, `src/log.zig`, `src/transport.zig` — these are imported by the emitted source so Zig's module resolution must find them at their workspace paths. The self_exe_path needed by `Compilation.Directories.init` can be obtained via `std.fs.selfExePathAlloc(arena)`. The `zig_lib_dir` needed by `Compilation.Directories.init` must come from the host Zig installation, not the vendored source — use `std.fs.selfExePathAlloc` + Zig's introspect to locate it, or use `zc.introspect.findZigLibDir` if available.
**Notes:** Zig 0.15.x `build.zig.zon` `.name` field requires a bare identifier (no dashes, no `@"..."` syntax). Used `zig_compiler` (underscore) for both the package name and the module name throughout; the `b.dependency` key must match the `dependencies` field name in the root zon (also `zig_compiler`). A `fingerprint` field in the root `build.zig.zon` is required by Zig 0.15.x; Zig provided the suggested value `0x9bbc06004e9bd75a` on first error. The vendor path dep (`vendor/zig-compiler`) does NOT require a fingerprint in its own `build.zig.zon`. The `zig_compiler` module is NOT compiled during `zig build test` unless source code actually `@import`s it — this is intentional for Phase 1. Phase 2 will add the actual `@import("zig_compiler")` usage in `cmd/circ-compile/main.zig`.

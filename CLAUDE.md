...
asserting from memory. When proposing a change, show the diff (or a minimal
...
| `zig build test` | Fast suite (dev-loop default), aggregated from many per-module `addTest` artifacts in `build.zig`. Excludes the slow emit-zig behavioral smoke; `tests/e2e/serializer_fixtures_test.zig` still covers circuit behavior on the production path in full. |
...
| `zig build bench` | Engine benchmark over the truth-table fixture corpus, compared against `tests/fixtures/bench/engine.bench.golden`. The benchmark asserts counters, not wall-clock. |
| `zig build parser:gen` | Regenerates `lib/parser/parser.zig` from `lib/grammar/proto-circ.peg`. Run it only when the grammar changes. It requires the langlang fork (`v0.0.13-zig.2`) on `PATH` and refuses any other version (upstream `go/v0.0.12` fails with `Output language \`zig\` not supported`). |
...
| `zig build libcirc-wasm` | Builds the same C ABI for `wasm32-freestanding`: `zig-out/lib/libcirc.wasm` (ten `circ_*` exports + `memory`; follows `-Dwasm-optimize`). `tests/e2e/libcirc_wasm_test.zig` drives it through Node. |
...
Hard errors block emission; the compiler never produces partial or "best-effort" artifacts. `--warnings-as-errors` (alias `-Werror`) promotes warnings.
...
- **Sub-circuits are fully flattened at runtime.** The `.min` blob is a flat ordered list of primitive components and connections; the runtime sees no hierarchy. The `.full` blob carries per-file IDs, port names, aliases, and macro provenance for tooling that needs human-readable structure.
- **The runtime is prebuilt and embedded.** The build compiles `templates/main.zig` and `templates/interpreter.zig` once into `zig-out/lib/circ-runtime.wasm`, then `@embedFile`s the result into the CLI as `runtime_embed`. End users never need a Zig toolchain at runtime.
...
6. **The allocator is global, not parameterised.** Allocations route through `memory.allocator` from `lib/memory.zig`: an `ArenaAllocator` over `page_allocator` on every target, so `free` only rewinds the most recent allocation and repeated allocation grows linear memory monotonically (allocate once and reuse, as the runtime's per-memory staging buffers do). `memory.reset()` (`arena.reset(.free_all)`) exists for libcirc and runs only when no `Circuit`/`Session` is alive. Do not add an allocator parameter to engine functions.
...
`build.zig` wires the compile-time switch `COLLECT_METRICS` per consumer: `false` for native and WASM, `true` only for `zig build bench`. When false, `Circuit.metrics` is `void` and the compiler strips every counter bump as dead code, so production and test builds are byte-identical to a metrics-free engine.
...
To add a test: place the `.circ` in `circuits/`, write the expected artifact in the matching `expected-*` directory, and reference it by stable name in the test. Regenerate with `UPDATE_GOLDENS=1 zig build test` and diff the result before committing.

## Finding the active work

This file lives on `main` and does not track in-progress initiatives. Before making changes, orient on the work in progress:

- `git status` and `git log -20 --oneline` for the current branch and recent commits. Branches take their initiative's name (`memories`, `libcirc`, `playground`, `layout`) and stack on each other as nested PRs.
...
2. **Stage by path, propose every commit message, then wait.** Stage by name only the files this change touches (`git add path/to/file ...`). Never use `git add -A`, `git add .`, or `git commit -a`: the human may have unrelated work in the tree, and a blanket add would sweep it into the commit. After staging, post the full proposed commit message in the conversation and wait for explicit approval before running `git commit`. Do not amend without fresh approval; if a hook fails, fix the issue, re-stage the same paths, and propose a new commit.
...
- Types in use: `feat`, `perf`, `docs`, `chore`, `ci`. Extend with other standard types (`fix`, `refactor`, `test`, `build`) when they fit.
- Scopes in use: `engine`, `bench`, `ci`, `site`. For changes bounded to one directory under `lib/`, the directory name is a natural scope (`parser`, `validator`, `topology`, `resolver`, `emit`, `preview`, `cli`).
...
Never use stage or phase prefixes (`S1.2:`, `Phase 3 Slice 3.4`, and the like) in commit titles. Branches encode stage; the commit history on `main` should not.
...
- **Body**: name the decision or the outcome (e.g. "move wire state off `Component` into a width-tiered pool", "bump topology format to v02"), not the staging plan that produced it. Reference the relevant plan file in `DOCS/` if a reader would want the wider context, but do not narrate the multi-PR roadmap inside any one PR. Omit context irrelevant to the diff.
...
- `tools/bench/main.zig`: bench harness; it holds the hand-maintained fixture-to-circuit mapping.

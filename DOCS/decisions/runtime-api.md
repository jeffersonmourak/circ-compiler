...
**Failure boundary.** Each settle allows 1,000,000 event-pop/downstream-evaluation work units. Exhaustion poisons the runtime until re-instantiation; void drive/run exports trap, and the additive `getSimulationStatus()` reports `1`. This prevents older hosts from silently accepting partial results while newer hosts can identify `E_NOSETTLE` separately from other traps. The playground uses the renderer's public constructor with that status query, preserving one shared runtime for canvas, Data, console, and agent actions. Gate-based latches remain legal. The budget is not a hard wall-clock deadline or a static proof of oscillation.

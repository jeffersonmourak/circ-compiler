...
## 2026-09-11 — Phase 6 — Run the active source file

**What shipped:** File selection now chooses the playground's compiler entry point. All sibling files remain available for imports. Canvas simulation, Schematic, Truth/copies, pin and memory metadata, source linking, console titles and multi-file downloads follow the selected file. Switching invalidates old requests immediately, discards the previous circuit and its runtime state, and rebuilds; source order and editor contents stay intact. The selected file is persisted.
**Files touched:** `site/src/components/Playground.astro`, `site/src/scripts/pipeline.ts`, `site/src/utils/split-files.ts`, `site/test/{island-smoke,libcirc,split-files}.test.ts`, `DOCS/decisions/{playground-bench,index}.md`, `DOCS/sim-protocol.md`, `DOCS/PLANS_PROMPT.md`, `DOCS/STATUS.md`
**Tests:** Added explicit-entry overlay coverage; real compiler/runtime/layout tests for the full-adder and imported half-adder; a nested sibling import with a broken parent; and an island walk with delayed parent replies, independent pin interaction, active-file source highlighting, Truth/CSV, filenames and saved selection. All four gates pass: 613 tests, typecheck, fresh build and bundle; `/playground` 133.7 KB raw / 46.5 KB gzip. No Chrome setup.
**Next slice:** Phase 7, on the human's word, or the next reported multi-file issue.
**Notes:** The old page always requested the last file and explicitly skipped rebuilding on file selection. `requestFor` now accepts an optional entry name; content compilation keeps its last-file default. The race test also exposed an async tint reading a cleared Truth table, so tinting, rendering and row activation now guard their target after awaits. The test uses the committed compiler WASM and renderer layout directly, with held worker-style replies in the island harness.

## 2026-09-11 — Phase 6 — Source-owned ROM images reach imports

**What shipped:** ROM images retain project/file/declaration ownership across source switches and initialize each imported instance by runtime ID. Build and reset apply the same images. Truth and its copies use a scratch session for imported images, enumerating the complete table below the cap. The existing version-2 envelope saves images within the space left after sources; oversized images remain available in memory and report their reload limit. Rename/delete and project-copy paths preserve image ownership.
**Files touched:** `site/src/components/Playground.astro`, `site/src/scripts/{sim-session,truth-view}.ts`, `site/src/utils/{source-images,playground-store}.ts`, `site/test/{source-images,island-smoke,playground-store}.test.ts`, `DOCS/decisions/{playground-bench,index}.md`, `DOCS/sim-protocol.md`, `DOCS/PLANS_PROMPT.md`, `DOCS/STATUS.md`
**Tests:** The reported `test.circ` → `main.circ` case passes through the built island, including return to the child, reset, complete Truth/CSV and saved image text. Real compiler/runtime coverage proves repeated nested instances, same-named declarations in distinct files, independent memory copies, invalid/empty images and reset; storage coverage proves defaults, normalization, round-trip and oversized-image omission. All four gates pass: 616 tests, typecheck, fresh build and bundle; `/playground` 137.2 KB raw / 47.7 KB gzip. No Chrome run.
**Next slice:** Review this fix; Phase 7 still awaits the human's word.
**Notes:** The imported-file mapping belongs to the compiled request, not the latest editor analysis. Matching concurrent analyses share a pending promise, and mapping changes invalidate an otherwise byte-identical artifact. Root-only console memory semantics remain intact. The compiler and renderer were not changed.
**Review note:** After the successful full gate, a concurrent edit to `site/src/styles/global.css` introduced a literal red focus border. A targeted `bun test test/bench-tokens.test.ts` then reported one failure. That edit is outside this fix and remains untouched.
**Approval:** The human approved the commit and requested inclusion of their CSS edit. Its final form adds `outline: none` to `.pg-switch-query`; the red border was removed. Included `site/src/styles/global.css` and reran the token/layout tests (14 pass), production build and bundle check successfully.

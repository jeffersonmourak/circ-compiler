...
pub fn emitRuntimeExports(allocator: std.mem.Allocator, module: *const ir.Module) ![]u8 {
    ...
    try writer.writeLine("}");
    try writer.writeLine("");
    try writer.writeLine("export fn setPin(component_id: i32, value: i64, defined: i64) void {");
    writer.indent();
    try writer.writeLine("if (!runtime_initialized) return;");
    ...
    try writer.writeLine("}");
    try writer.writeLine("");
    try writer.writeLine("export fn getOutputValue(component_id: i32) i64 {");
    writer.indent();
    try writer.writeLine("if (!runtime_initialized) return 0;");
    try writer.writeLine("if (component_id < 0) return 0;");
    try writer.writeLine("const id: u32 = @intCast(component_id);");
    try writer.writeLine("if (id >= component_table.len) return 0;");
    try writer.writeLine("if (!containsId(output_component_ids, id)) return 0;");
    try writer.writeLine("return @bitCast(runtime_circuit.readState(component_table[id].state_handle).value);");
    writer.dedent();
    try writer.writeLine("}");
    try writer.writeLine("");
    try writer.writeLine("export fn getOutputDefined(component_id: i32) i64 {");
    writer.indent();
    try writer.writeLine("if (!runtime_initialized) return 0;");
    try writer.writeLine("if (component_id < 0) return 0;");
    try writer.writeLine("const id: u32 = @intCast(component_id);");
    try writer.writeLine("if (id >= component_table.len) return 0;");
    try writer.writeLine("if (!containsId(output_component_ids, id)) return 0;");
    try writer.writeLine("return @bitCast(runtime_circuit.readState(component_table[id].state_handle).defined);");
    writer.dedent();
    try writer.writeLine("}");
    try writer.writeLine("");

...
function emitLandingTwin(): void {
  const lines = [
    ...
    '```',
    '',
    `[Edit this in the playground](${SITE_URL}/playground#pick=example:half-adder)`,
    '',
    '## Six primitives. Five macros. One import.',
    '',
    "A program is a flat list of declarations — name a pin, place a gate, bind its ports. It stops where the early Nand2Tetris hardware chapters stop: enough to build a CPU from NAND, nothing you'd need a datasheet for.",
    '',
    '- Primitives: `and`, `not`, `led`, `wire`, `rom`, `ram`.',
    '- Macros: `or`, `nand`, `nor`, `xor`, `xnor` — expand to primitives.',
    '- Signals: 1–64 bits, `input[4] a`, `a[0..2]`, `{a, b}`.',
    '- Files: `import adder "adder.circ"` — one circuit each.',
    '',
    ...
    '',
    '## Source on the left. What the compiler saw on the right.',
    '',
    'A half-adder. Two inputs, an XOR macro for the sum, an AND for the carry. Every program can print itself like this from the terminal — the same picture the live card above is animating.',
    '',
    '```circ',
    HERO_SOURCE.trimEnd(),
    '```',
    ...
    '```',
    '',
    `[Open it in the playground](${SITE_URL}/playground#pick=example:half-adder)`,
    '',
    '## From the gallery',
    '',
    `[two-bit-adder](${SITE_URL}/gallery.md): Two 2-bit operands, sliced into lanes, added with a ripple carry, joined back into a bus.`,
    '',
    `[four-bit-adder](${SITE_URL}/gallery.md): The same adder twice as wide: four lanes, each carrying into the next, 256 rows of truth.`,
    '',
    `[rom-lookup](${SITE_URL}/gallery.md): A lookup table the host fills at load. Drive the address and read the word; the console can peek and poke it.`,
    '',
    `[All examples](${SITE_URL}/gallery.md)`,
    '',
    '## Install',
    '',
...
function emitLlmsTxt(): void {
  const lines = [
    ...
    '## Docs',
    '',
    `- [Landing page](${SITE_URL}/index.md): A live half-adder, the language vocabulary, the compile-and-run steps, the --preview schematic, and three gallery examples.`,
